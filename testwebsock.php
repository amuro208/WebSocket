...
class CMDServer extends WebSocketServer {
  ...
  protected function process ($from, $data) { // changed $user to $from, just to avoid confusion
	...
	if(preg_match("/CLIENT/",$data)){
      ...
      $ndx = intval($ndxStr);
      if(array_key_exists ($ndx,$this->users)){
         $this->printClientList();
          echo "CLIENT ".$ndx." ALREADY EXIST IN TCS ARRAY\n";//.$this->tcs_users[$ndx]->$ip."\n";
        //  $msg = "ALL#DECLINED#".$ndx."#NOTICE#EOF";
        //  $this->send($from, $msg);

      }

        $from->tcs_id = $ndx;
      //  $this->tcs_users[$ndx] = $from;
       $this->printClientList();
        foreach ($this->users as $user) {
          try{
            if($user == $from){
      ...
      //  echo "??????? : ".$data."\n";
				...
				if(array_search($from,$this->users)){
					...
					if($ndx === "ALL"){
						...
						if(isset($this->users[$ndx])){
							try{
								$this->send($this->users[$ndx], $data);
							}catch(Exception $e){
								unset($this->users[$ndx]);
								$this->printClientList();
							}
					   }
					}
